...
export class Database {
  ...
  async getAllPositions(): Promise<Position[]> {
    return new Promise((resolve, reject) => {
      ...
      this.db.all('SELECT * FROM positions ORDER BY id', (err, rows: any[]) => {
        if (err) {
          reject(err);
        } else {
          resolve(rows as Position[]);
        }
      });
    });
  }
  ...
  // Update position
  async updatePosition(id: number, result: string): Promise<void> {
    return this.runQuery('UPDATE positions SET result = ? WHERE id = ?', [result, id]);
  }

  // Reset all positions
  async resetAllPositions(): Promise<void> {
