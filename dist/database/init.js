...
function initDatabase() {
    return new Promise((resolve, reject) => {
      ...
      `, (err) => {
                if (err) {
                    console.error('❌ Помилка створення таблиці achievements:', err);
                    reject(err);
                    return;
                }
                console.log('✅ Таблиця achievements готова');
            });
            // Вставка дефолтних досягнень
            const achievements = [
                'first-click',
                'breakeven',
                'in-profit',
            ...
            const insertAchievement = db.prepare('INSERT OR IGNORE INTO achievements (code, unlocked) VALUES (?, 0)');
            achievements.forEach(code => {
                insertAchievement.run(code);
            });
            insertAchievement.finalize();
            console.log('✅ Дефолтні досягнення створені');
            resolve(db);
        });
    });
}
